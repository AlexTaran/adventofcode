...
fn find_paths(g: &HashMap<String, Vec<String>>, v: String, visited: HashSet<String>) -> u32 {
    if v == "end" {
        return 1;
    }
    ...
    for neib in neibs {
        if newvis.contains(neib) {
            continue;
        }
        counter += find_paths(g, neib.to_string(), newvis.clone());
    }
    return counter;
}
...
fn main() {
    ...
    println!("{}", find_paths(&g, "start".to_string(), HashSet::new()));
}
