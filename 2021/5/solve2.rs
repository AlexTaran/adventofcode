...
fn main() {
    ...
    loop {
        ...
        let p1:Vec<i32> = spl[0].trim().split(",").map(|x|x.parse::<i32>().unwrap()).collect();
        let p2:Vec<i32> = spl[1].trim().split(",").map(|x|x.parse::<i32>().unwrap()).collect();
        if p1[0] == p2[0] {
            ...
            let dlt0 = (p2[0] - p1[0]).signum();
            let dlt1 = (p2[1] - p1[1]).signum();
            let mut c0 = p1[0];
            let mut c1 = p1[1];
            loop {
                field[c0 as usize][c1 as usize] += 1;
                if c0 == p2[0] && c1 == p2[1] {
                    break;
                }
                c0 += dlt0;
