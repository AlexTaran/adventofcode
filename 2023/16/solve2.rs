...
fn calc(table:&Vec<Vec<char>>, start: V2, dr: DIR) -> u32 {
    let w = table[0].len();
    let h = table.len();
...
fn main() {
    ...
    let mut res = 0;
    for i in 0..h {
        res = std::cmp::max(res, calc(&table, V2{x: 0, y: i as i32}, DIR::Right));
        res = std::cmp::max(res, calc(&table, V2{x: w as i32 - 1, y: i as i32}, DIR::Left));
    }
    for i in 0..w {
        res = std::cmp::max(res, calc(&table, V2{x: i as i32, y: 0}, DIR::Down));
        res = std::cmp::max(res, calc(&table, V2{x: i as i32, y: h as i32 - 1}, DIR::Up));
    }
    println!("{}", res);
}
